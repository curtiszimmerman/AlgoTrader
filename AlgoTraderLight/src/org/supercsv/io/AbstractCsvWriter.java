...
/**
 * The writer class capable of writing arrays, maps,... to a CSV file. Notice
 * that the cell processors can also be
 * utilized when writing. E.g. they can help ensure that only numbers are
 * written in numeric columns, that numbers are
 * unique or the output does not contain certain characters or exceed specified
 * string lengths.
 * 
...
public abstract class AbstractCsvWriter implements ICsvWriter {
	...
	protected AbstractCsvWriter(final Writer stream,
	...
	/**
	 * Make a string ready for writing by escaping various characters as
	 * specified by the CSV format
	 * 
	 * @param csvElem
	 ...
	 */
	protected void write(final List<? extends Object> content)
	        throws IOException {
	...
	protected void write(final Object... content) throws IOException {
		...
		for (final Object o : content) {
			if (o == null) { throw new NullInputException(
			        "Object at position " + i + " is null", new CSVContext(
			                getLineNumber(), i),
			        (Throwable) null); }
			strarr[i++] = o.toString();
		}
		...
		switch (content.length) {
			case 0:
				throw new SuperCSVException(
				        "There is no content to write for line " +
				                getLineNumber(), new CSVContext(
				                getLineNumber(), 0));
