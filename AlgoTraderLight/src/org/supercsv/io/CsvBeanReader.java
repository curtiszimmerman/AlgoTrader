...
public class CsvBeanReader extends AbstractCsvReader implements ICsvBeanReader {
	/**
	 * object used for storing intermediate result of a processing of cell
	 * processors and before put into maps/objects etc..
	...
	/**
	 * Create a csv reader with a specific preference. Note that the
	 * <tt>reader</tt> provided in the argument will be
	...
	/**
	 * Creates an object of the type or if it is an interface, create a proxy
	 * instance implementing the interface type.
	 * 
	 * @param clazz
	 *            the type to instantiate. If the type is a class type, an
	 *            instance can be created straight away. If the
	 *            type is an interface type, a proxy is created on the fly which
	 *            acts as an implementation.
	 * @param nameMapping
	 ...
	 */
	<T> T fillObject(final Class<T> clazz, final String[] nameMapping)
		...
		try {
			...
			// map results into an object by traversing the list of nameMapping
			// and for each non-null,
			// map that name to an entry in the lineResult
			// map results to the setter methods
			for (int i = 0; i < nameMapping.length; i++) {
				// don't call a set-method in the bean, if there is no result to
				// store
				if (nameMapping[i] == null) {
					continue;
				}
				try {
					// System.out.println(String.format("mapping[i]= %s, lR[%d] = %s val '%s'",
					// nameMapping[i], i,
					// lineResult
					// .get(i).getClass(), lineResult.get(i)));
					if (lineResult.get(i) != null) {
						cache.getSetMethod(resultBean, nameMapping[i],
						        lineResult.get(i).getClass())//
						        .invoke(resultBean, lineResult.get(i));
					}
				} catch (final IllegalArgumentException e) {
					throw new SuperCSVException("Method set" +
					        nameMapping[i].substring(0, 1).toUpperCase()
					        + nameMapping[i].substring(1) +
					        "() does not accept input \"" + lineResult.get(i) +
					        "\" of type "
			...
			throw new SuperCSVReflectionException(
			        "Error while filling an object", e);
	 ...
	 */
	@Override
	public <T> T read(final Class<T> clazz, final String... nameMapping)
	...
	public <T> T read(final Class<T> clazz, final String[] nameMapping,
		...
		if (tokenizer.readStringList(super.line)) {
			Util.processStringList(lineResult, super.line, processors,
